...
class CarouselDots extends Component {
  ...
  renderBorderedSvg = (i, active) => {
    ...
    return (
      <svg key={i} height={proportions} width={proportions}>
        <circle
          onClick={() => {
            this.dotClicked(i);
          }}
  ...
  renderBorderlessSvg = (i, active) => {
    ...
    let fill = active ? this.state.activeFill : this.state.inactiveFill;
    return (
      <svg key={i} eight={proportions} width={proportions}>
        <circle
          onClick={() => {
            this.dotClicked(i);
          }}
  ...
  generateDots = () => {
    ...
    for (let i = 1; i <= numberOfDots; i++) {
      let dot = border
        ? this.renderBorderedSvg(i, i == activeLocation)
        : this.renderBorderlessSvg(i, i == activeLocation);
      dots.push(dot);
    }
