...
describe("CarouselDots component", () => {
  ...
  it("Component should render the number of dots supplied in the properties", () => {
    ...
    const component = mount(
      <CarouselDots numberOfDots={3} activeLocation={1} onStatusChange={jest.fn()} />
    );
  ...
  it("Component should switch from dot 1 to dot 2 upon clicking 2nd dot", () => {
    ...
    const component = mount(
      <CarouselDots numberOfDots={3} activeLocation={1} onStatusChange={callback} />
    );
    component
      .find(".CarouselDot")
      .at(1)
    ...
    expect(component.state().activeLocation).toBe(1);
  });

  it("Component should render border when prop `border` is set ", () => {
    const component = mount(
      <CarouselDots numberOfDots={3} activeLocation={0} border onStatusChange={jest.fn()} />
    );
